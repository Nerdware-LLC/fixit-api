 ...
 */
export const resetPassword = mwAsyncCatchWrapper<RestApiRequestBodyByPath["/auth/password-reset"]>(
  async (req, res) => {
    // Retrieve the password-reset token from the request body
    const { password: newPassword, passwordResetToken } = req.body;
