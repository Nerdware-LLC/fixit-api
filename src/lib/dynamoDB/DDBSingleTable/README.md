...
**DDB Single Table** is a modeling tool and client wrapper for DynamoDB which aims to serve as a [single-table-first](#q-why-use-just-a-single-table) replacement for tools like [dynamoose](https://dynamoosejs.com/getting_started/Introduction) which are fundamentally designed to work with multiple tables, and tend to have features that break when workarounds are implemented which try to force single-table usage.
...
- Easy to use API for managing DDB tables, connections, and models
...
- Easy access to a streamlined DynamoDB client (more info [here](#q-how-does-the-ddb-st-api-interact-with-the-underlying-dynamodb-client))
