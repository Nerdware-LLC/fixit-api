import { UserSubscription, type UserSubscriptionType } from "@models/UserSubscription";
...
export const resolvers: Partial<Resolvers> = {
  Query: {
    mySubscription: async (parent, args, { user }) => {
      ...
      let subToReturn: UserSubscriptionType | undefined;
      ...
      subs.forEach((currentSub) => {
        ...
        */
        ...
        } else if (currentSub.status === "active" && (subToReturn.status !== "active" || wasCreatedEarlier(currentSub, subToReturn))) {
      ...
      return subToReturn as any; // FIXME
    }
  }
};
...
const wasCreatedEarlier = (
  { createdAt: createdAt_1 }: UserSubscriptionType,
  { createdAt: createdAt_2 }: UserSubscriptionType
) => {
  return createdAt_1 < createdAt_2;
};

const wasUpdatedLater = (
  { updatedAt: updatedAt_1 }: UserSubscriptionType,
  { updatedAt: updatedAt_2 }: UserSubscriptionType
) => {
