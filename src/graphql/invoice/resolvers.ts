    ...
    payInvoice: async (parent, { invoiceID }, { user }) => {
      ...
      if (!payingUser.stripeConnectAccount?.id) {
        throw new GqlForbiddenError(
          "Invalid Stripe Connect account - please review your Stripe account settings and try again."
        );
      }

      if (stripeCustomer.deleted) {
        throw new GqlForbiddenError(
          "Invalid Stripe Customer - please review your Stripe account settings and try again."
        );
      }
